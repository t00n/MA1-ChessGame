...
void main(void) {
    ...
    float s10 = luminance(texture(u_render_texture, vec2(x + -dx,y+  0.0)).rgb);
    float s20 = luminance(texture(u_render_texture, vec2(x + -dx,y+ -dy)).rgb);
    float s01 = luminance(texture(u_render_texture, vec2(x + 0.0,y+  dy)).rgb);
    float s21 = luminance(texture(u_render_texture, vec2(x + 0.0,y+  -dy)).rgb);
    float s02 = luminance(texture(u_render_texture, vec2(x + dx, y+ dy)).rgb);
    float s12 = luminance(texture(u_render_texture, vec2(x + dx, y+ 0.0)).rgb);
